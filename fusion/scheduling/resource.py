...
class Resource(object):
    ...
    def __init__(self, buf_capacity_list, buf_access_cost_list,
                 ...
                 invalid_underutilized=True, precision=16, pipeline=False):
        ...
        if type(buf_access_cost_list[0]) is not list:
            self.access_cost = [[x*1.0] * 3 for x in buf_access_cost_list]
    ...
    def arch(cls, info):
        return cls(info["capacity"], info["access_cost"], info["static_cost"],
                   ...
                   info["memory_partitions"], info['invalid_underutilized'], info['precision'])
